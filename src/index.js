...
module.exports = function toReadable (number) {
    if(number == 0) {
      ...
      if(str1.length < 4) {
        return forReadable(number).trim();
       } else if(str1.length < 7) {
         let out = forReadable(number / 1000) + " thousand " + forReadable(number%1000);
         return out.trim();
       } else {
         let out = forReadable(number / 1000000) + " million " + forReadable(number/1000) ; if(forReadable(number / 1000) != "") {out += ' thousand ' ;}
         out += forReadable(number%1000);
         return out.trim();
       }   
}
